...
import { User, Eye, EyeOff, Activity, Lock } from 'lucide-react';
...
export const PatientLoginPage: React.FC = () => {
  ...
  const [emailOrPhone, setEmailOrPhone] = useState('rahul.kumar@example.com');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      await login(emailOrPhone, 'patient');
    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-gradient-to-br from-[#D7EAEE] via-[#F7FBFC] to-[#9FD8E1]/40">
      <div className="w-full max-w-md space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-xs font-semibold text-slate-500">Patient Portal Sign In</p>
        </div>
        ...
        <Card className="p-8 space-y-5">
          <div className="flex items-center gap-3 pb-3 border-b border-[#D7EAEE]">
            ...
            <div>
              <h2 className="text-base font-bold text-[#102A43]">Patient Login</h2>
              <p className="text-xs text-slate-500">Enter your clinical account details</p>
            </div>
          </div>
          ...
          <div className="text-center pt-3 border-t border-[#D7EAEE] text-xs text-slate-500">
            ...
            <Link to="/signup/patient" className="font-bold text-[#1D837F] hover:underline">
              {t('auth.signUp')}
            </Link>
          </div>
        </Card>
